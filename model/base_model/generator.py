import json
import torch
from torch import nn
from torch.nn import ModuleList
from model.PositionalEncoding import LearnedPositionalEncoding, FixedPositionalEncoding
from model.module.HCABL import HybridCNNAttentionBottleneck
from model.module.ScConv import ScConv, SRU
from model.module.TCAM import ChannelTransformer
from model.block import from_rgb, to_rgb, conv_block, up_conv, CCA


class Generator(nn.Module):

    def __init__(self, params):
        super(Generator, self).__init__()

        self.img_dim = params['img_dim']
        self.patch_dim = params['patch_dim']
        self.embedding_dim = params['embedding_dim']
        self.num_channels = params['num_channels']
        self.num_heads = params['num_heads']
        self.num_layers = params['num_layers']
        self.hidden_dim = params['hidden_dim']
        self.dropout_rate = params['dropout_rate']
        self.attn_dropout_rate = params['attn_dropout_rate']
        self.in_ch = params['in_ch']
        self.out_ch = params['out_ch']
        self.conv_patch_representation = params['conv_patch_representation']
        assert self.embedding_dim % self.num_heads == 0
        assert self.img_dim % self.patch_dim == 0
        self.num_patches = int((self.img_dim // self.patch_dim) ** 2)
        self.seq_length = self.num_patches
        self.flatten_dim = 128 * self.num_channels
        self.linear_encoding = nn.Linear(self.flatten_dim, self.embedding_dim)
        self.positional_encoding_type = params["positional_encoding_type"]
        if self.positional_encoding_type == "learned":
            self.position_encoding = LearnedPositionalEncoding(
                self.seq_length, self.embedding_dim, self.seq_length
            )
        elif self.positional_encoding_type == "fixed":
            self.position_encoding = FixedPositionalEncoding(
                self.embedding_dim,
            )
        self.pe_dropout = nn.Dropout(p=self.dropout_rate)
        self.hcabl = HybridCNNAttentionBottleneck(
            self.embedding_dim,
            self.num_layers,
            self.num_heads,
            self.hidden_dim,
            self.dropout_rate,
            self.attn_dropout_rate,
        )

        self.pre_head_ln = nn.LayerNorm(
            self.embedding_dim)
        if self.conv_patch_representation:
            self.Conv_x = nn.Conv2d(
                256,
                self.embedding_dim,
                kernel_size=3,
                stride=1,
                padding=1
            )
        self.bn = nn.BatchNorm2d(256)
        self.relu = nn.ReLU(inplace=True)
        self.rgb_to_feature = ModuleList([from_rgb(32), from_rgb(64), from_rgb(128)])
        self.feature_to_rgb = ModuleList([to_rgb(32), to_rgb(64), to_rgb(128), to_rgb(256)])
        self.Maxpool = nn.MaxPool2d(kernel_size=2, stride=2)
        self.Maxpool1 = nn.MaxPool2d(kernel_size=2, stride=2)
        self.Maxpool2 = nn.MaxPool2d(kernel_size=2, stride=2)
        self.Maxpool3 = nn.MaxPool2d(kernel_size=2, stride=2)
        self.Maxpool4 = nn.MaxPool2d(kernel_size=2, stride=2)
        self.Conv1 = conv_block(self.in_ch, 16)
        self.Conv1_1 = conv_block(16, 32)
        self.Conv2 = conv_block(32, 32)
        self.Conv2_1 = conv_block(32, 64)
        self.Conv3 = conv_block(64, 64)
        self.Conv3_1 = conv_block(64, 128)
        self.Conv4 = conv_block(128, 128)
        self.Conv4_1 = conv_block(128, 256)
        self.Conv5 = conv_block(512, 256)
        self.scconv = ScConv(256)
        self.tcam = ChannelTransformer(channel_num=[32, 64, 128, 256],
                                       patchSize=[32, 16, 8, 4])
        self.Up5 = up_conv(256, 256)
        self.coatt5 = CCA(F_g=256, F_x=256)
        self.Up_conv5 = conv_block(512, 256)
        self.Up_conv5_1 = conv_block(256, 256)
        self.Up4 = up_conv(256, 128)
        self.coatt4 = CCA(F_g=128, F_x=128)
        self.Up_conv4 = conv_block(256, 128)
        self.Up_conv4_1 = conv_block(128, 128)
        self.Up3 = up_conv(128, 64)
        self.coatt3 = CCA(F_g=64, F_x=64)
        self.Up_conv3 = conv_block(128, 64)
        self.Up_conv3_1 = conv_block(64, 64)
        self.Up2 = up_conv(64, 32)
        self.coatt2 = CCA(F_g=32, F_x=32)
        self.Up_conv2 = conv_block(64, 32)
        self.Up_conv2_1 = conv_block(32, 32)
        self.Conv = ScConv(32, self.out_ch)

    def reshape_output(self, x):
        x = x.view(
            x.size(0),
            int(self.img_dim / self.patch_dim),
            int(self.img_dim / self.patch_dim),
            self.embedding_dim,
        )
        x = x.permute(0, 3, 1, 2).contiguous()

        return x

    def forward(self, x):
        output = []
        x_1 = self.Maxpool(x)
        x_2 = self.Maxpool(x_1)
        x_3 = self.Maxpool(x_2)
        e1 = self.Conv1(x)
        e1 = self.Conv1_1(e1)
        e2 = self.Maxpool1(e1)
        x_1 = self.rgb_to_feature[0](x_1)
        e2 = x_1 + e2
        e2 = self.Conv2(e2)
        e2 = self.Conv2_1(e2)
        e3 = self.Maxpool2(e2)
        x_2 = self.rgb_to_feature[1](x_2)
        e3 = x_2 + e3
        e3 = self.Conv3(e3)
        e3 = self.Conv3_1(e3)
        e4 = self.Maxpool3(e3)
        x_3 = self.rgb_to_feature[2](x_3)
        e4 = x_3 + e4
        e4 = self.Conv4(e4)
        e4 = self.Conv4_1(e4)
        e5 = self.Maxpool4(e4)
        e5 = self.scconv(e5)
        e1, e2, e3, e4, att_weights = self.tcam(e1, e2, e3, e4)
        residual = e5
        e5 = self.bn(e5)
        e5 = self.relu(e5)
        e5 = self.Conv_x(e5)
        e5 = e5.permute(0, 2, 3, 1).contiguous()
        e5 = e5.view(e5.size(0), -1, self.embedding_dim)
        e5 = self.position_encoding(e5)
        e5 = self.pe_dropout(e5)
        e5 = self.hcabl(e5)
        e5 = self.pre_head_ln(e5)
        e5 = self.reshape_output(e5)
        e5 = self.Conv5(e5)
        e5 = e5 + residual
        d5 = self.Up5(e5)
        e4_att = self.coatt5(g=d5, x=e4)
        d5 = torch.cat((e4_att, d5), dim=1)
        d5 = self.Up_conv5(d5)
        d5 = self.Up_conv5_1(d5)
        out3 = self.feature_to_rgb[3](d5)
        output.append(out3)
        d4 = self.Up4(d5)
        e3_att = self.coatt4(g=d4, x=e3)
        d4 = torch.cat((e3_att, d4), dim=1)
        d4 = self.Up_conv4(d4)
        d4 = self.Up_conv4_1(d4)
        out2 = self.feature_to_rgb[2](d4)
        output.append(out2)
        d3 = self.Up3(d4)
        e2_att = self.coatt3(g=d3, x=e2)
        d3 = torch.cat((e2_att, d3), dim=1)
        d3 = self.Up_conv3(d3)
        d3 = self.Up_conv3_1(d3)
        out1 = self.feature_to_rgb[1](d3)
        output.append(out1)
        d2 = self.Up2(d3)
        e1_att = self.coatt2(g=d2, x=e1)
        d2 = torch.cat((e1_att, d2), dim=1)
        d2 = self.Up_conv2(d2)
        d2 = self.Up_conv2_1(d2)
        out0 = self.feature_to_rgb[0](d2)
        output.append(out0)
        return output
